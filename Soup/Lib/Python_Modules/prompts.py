...
def exit_program(tool_name: str) -> None:
    if tool_name == "main" or tool_name == "main_menu" or tool_name == "NA":
...
def menu_prompt(first: int, last: int, tool_name: str) -> int:
    while True:
        try:
            ...
            status_code = validator.check_user_choice(user_choice, first=first, last=last)
            if status_code == 0:
                return user_choice
        except ValueError:
...
def target_ipv4(tool_name: str) -> str:
    while True:
        try:
            ...
            status_code = validator.check_IPv4_target(ipv4_address)
            if status_code == 0:
                return ipv4_address
        except KeyboardInterrupt:
            exit_program(tool_name=tool_name)
...
def target_website(tool_name: str) -> str:
    while True:
        try:
            ...
            domain_name = validator.check_web_target(url)
            if domain_name[0] == 0:
...
def target_website_with_params_or_endpoints(tool_name: str) -> str:
    while True:
        try:
            url = str(input(f"{magenta}Please Enter A Target URL {green}[Ex: https://burgers.com/order or books.library.com/?page=100]{magenta}: {reset}"))

            target = validator.check_web_target_with_params_or_endpoints(url)
...
def port(tool_name: str) -> int:
    while True:
        try:
            ...
            status_code = validator.check_port(port)
            if status_code == 0:
                return int(port)
        except KeyboardInterrupt:
            exit_program(tool_name=tool_name)
...
def port_range(tool_name: str) -> list:
    while True:
        try:
            ...
            status_code = validator.check_port_range(port_range)
            if status_code == 0:
...
def timeout(tool_name: str) -> int:
    while True:
        try:
            ...
            validator_result = validator.check_timeout(timeout_amount)
    ...
    while True:
        try:
            ...
            if save == "y" or save == "Y" or save == "yes" or save == "YES":
                ...
                print(f"{magenta}[*] The tool output will be saved at {cyan}/Hackesoup/Saves/{tool_name}_{date}_{time_now}.json{reset}")
                time.sleep(3)
